...
class _ServerDetailScreenState extends State<ServerDetailScreen> {
        ...
        actions: [
          IconButton(
            onPressed: () => Navigator.of(context).push(
              MaterialPageRoute(
                builder: (_) =>
                    ServerFormScreen(store: widget.store, server: server),
              ),
            ),
        ...
        child: ListView(
          ...
          children: [
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(_firstIp(server.ipAddress),
                      style: const TextStyle(
                          fontSize: 16,
            ...
            if (isVm)
              _Section(
                ...
                rows: [
                  _InfoRow(
                      'Host Server',
                      widget.store.byId(server.parentId ?? '')?.name ??
                          'Dedicated host'),
                  _InfoRow('Operating System', server.operatingSystem),
                  _InfoRow('vCPU', server.vmCpuCores?.toString() ?? 'Not set'),
                  _InfoRow(
                      'Memory',
                      server.vmMemoryGb == null
                          ? 'Not set'
                          : '${_formatNumber(server.vmMemoryGb!)} GB'),
                  _InfoRow(
                      'Disk',
                      server.vmDiskGb == null
                          ? 'Not set'
                          : '${_formatNumber(server.vmDiskGb!)} GB'),
            ...
            if (isVm)
              ...
              _Section(
                ...
                rows: [
                  ...
                  _InfoRow(
                      'Operating System / Hypervisor', server.operatingSystem),
                  _InfoRow('Login User', server.loginUser),
                ],
              ),
              _Section(
                ...
                rows: [
                  ...
                  _InfoRow(
                      ...
                      server.clientPhone.isEmpty
                          ? 'Not added'
                          : server.clientPhone),
                  _InfoRow('Monthly Cost',
                      'PKR ${server.monthlyCost.toStringAsFixed(2)}'),
                  _InfoRow(
                      'Purchase Date', dateFormat.format(server.purchaseDate)),
                  _InfoRow(
            ...
            _Section(
              ...
              rows: [
                _InfoRow('Details',
                    server.notes.isEmpty ? 'No notes added.' : server.notes)
              ],
            ),
          ],
        ),
  ...
  Future<void> _confirmDelete(
      BuildContext context, ManagedServer server) async {
...
class _RenewalBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final text = days < 0
        ? 'Renewal overdue by ${days.abs()} day(s)'
