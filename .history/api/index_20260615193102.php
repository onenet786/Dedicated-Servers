...
function handle_auth(): void
{
      ...
      // Redrawn header and table to match attached JPG style
      $pdf = begin_pdf();

      // === TOP STRIPES (orange then green) ===
      rect_fill($pdf, 40, 820, 180, 8, '0.894 0.361 0'); // orange stripe left
      rect_fill($pdf, 320, 820, 252, 8, '0.427 0.812 0.318'); // green stripe right

      // Boxed 'Invoice' label centered with orange border and white background
      rect_fill($pdf, 260, 802, 92, 26, '1 1 1'); // white box
      line($pdf, 260, 828, 352, 828, '0.894 0.361 0');
      line($pdf, 260, 802, 352, 802, '0.894 0.361 0');
      line($pdf, 260, 802, 260, 828, '0.894 0.361 0');
      line($pdf, 352, 802, 352, 828, '0.894 0.361 0');
      text($pdf, 'Invoice', 296, 812, 12, '/F1B', '0.192 0.737 0.278');

      // Company name and info (left) and invoice meta (right)
      text($pdf, 'OneNet Solutions', 50, 790, 14, '/F1B', '0.192 0.737 0.278'); // green
      text($pdf, 'NTN# 1443348-6', 50, 774, 9, '/F1', '0 0 0');
      text($pdf, 'Shop No.1, 83-D, The Mall, Lahore', 50, 762, 9, '/F1', '0 0 0');
      text($pdf, '+923214424625', 50, 750, 9, '/F1', '0 0 0');

      text($pdf, 'Dated: ' . $renewalFormatted, 420, 790, 10, '/F1', '0 0 0');
      text($pdf, 'Invoice No.: ' . $invoiceNo, 420, 774, 10, '/F1', '0 0 0');

      // === BILL TO ===
      text($pdf, 'Bill To:', 50, 738, 10, '/F1', '0.192 0.737 0.278');
      text($pdf, $client, 50, 722, 12, '/F1B', '0 0 0');
      text($pdf, $name, 50, 706, 10, '/F1', '0 0 0');
      if ($location !== '') {
        text($pdf, $location, 50, 690, 10, '/F1', '0 0 0');
      }
      ...
      $col3 = 440;  // Unit Price (narrow)
      $col4 = 520;  // Total (narrow)
      $rowH = 24;

      // Header row with green labels
      rect_fill($pdf, $tableLeft, $tableTop - $rowH, $tableRight - $tableLeft, $rowH, '0.945 0.973 0.957'); // very light green
      text($pdf, 'Qty', $col1, $tableTop - 8, 9, '/F1B', '0.192 0.737 0.278');
      text($pdf, 'Description', $col2, $tableTop - 8, 9, '/F1B', '0.192 0.737 0.278');
      text($pdf, 'Unit Price', $col3, $tableTop - 8, 9, '/F1B', '0.192 0.737 0.278');
      text($pdf, 'Total', $col4, $tableTop - 8, 9, '/F1B', '0.192 0.737 0.278');

      // === SINGLE ROW ===
      $rowY = $tableTop - $rowH - 4;
      // large description area spans multiple lines
      rect_fill($pdf, $tableLeft, $rowY - 80, $tableRight - $tableLeft, 80, '1 1 1');
      text($pdf, '1', $col1, $rowY - 10, 10, '/F1', '0 0 0');
      text($pdf, 'Monthly Services Charges for the Month of ' . $monthYear, $col2, $rowY - 10, 10, '/F1', '0 0 0');
      text($pdf, 'PKR ' . $amount, $col3, $rowY - 10, 10, '/F1', '0 0 0');
      text($pdf, 'PKR ' . $amount, $col4, $rowY - 10, 10, '/F1', '0 0 0');

      // Grid lines (only outer and key verticals to mimic attached design)
      // Horizontal lines
      line($pdf, $tableLeft, $tableTop, $tableRight, $tableTop, '0 0 0');
      line($pdf, $tableLeft, $rowY - 80, $tableRight, $rowY - 80, '0 0 0');

      // Vertical lines
      line($pdf, $tableLeft, $tableTop, $tableLeft, $rowY - 80, '0 0 0');
      line($pdf, 85, $tableTop, 85, $rowY - 80, '0 0 0');
      line($pdf, $col3 - 10, $tableTop, $col3 - 10, $rowY - 80, '0 0 0');
      line($pdf, $col4 - 10, $tableTop, $col4 - 10, $rowY - 80, '0 0 0');
      line($pdf, $tableRight, $tableTop, $tableRight, $rowY - 80, '0 0 0');

      // === TOTALS (bottom-right) ===
      $totalY = $rowY - 92;
      text($pdf, 'Total', $col3 - 50, $totalY, 11, '/F1B', '0 0 0');
      text($pdf, 'PKR ' . $amount, $col4, $totalY, 11, '/F1B', '0 0 0');

      $grandY = $totalY - 20;
      rect_fill($pdf, $col3 - 60, $grandY - 8, ($tableRight - ($col3 - 60)), 18, '0.192 0.737 0.278'); // green
      text($pdf, 'Grand Total', $col3 - 40, $grandY, 11, '/F1B', '1 1 1');
      text($pdf, 'PKR ' . $amount, $col4, $grandY, 11, '/F1B', '1 1 1');

      // === FOOTER ===
      text($pdf, 'Thank you for your business.', 50, $grandY - 40, 10, '/F1', '0.4 0.4 0.4');
