...
{
    $date = new DateTimeImmutable($server['renewal_date'] ?? 'now');
    $renewalFormatted = $date->format('j F Y');
    $monthYear = $date->format('F Y');
    $invoiceNo = 'OneNetSol' . substr((string) ($server['id'] ?? 'invoice'), 0, 5);
    $amount = number_format((float) ($server['monthly_cost'] ?? 0), 0);
    $client = (string) ($server['assigned_client'] ?? 'Client');
    $location = (string) ($server['location'] ?? '');
    $spec = (string) ($server['specification'] ?? '');
    $ip = first_ip((string) ($server['ip_address'] ?? ''));
    $provider = (string) ($server['provider'] ?? '');

    $lines = [
        ['Invoice', 286, 810, 18],
        ['OneNet Solutions', 50, 782, 14],
        ['NTN# 1443348-6', 50, 763, 11],
        ['Shop No.1, 83-D, The Mall,', 50, 745, 11],
        ['Lahore', 50, 729, 11],
        ['+923214424625', 50, 713, 11],
        ['Dated: ' . $renewalFormatted, 50, 685, 11],
        ['Invoice No.: ' . $invoiceNo, 310, 685, 11],
        ['Bill To:', 50, 650, 11],
        [$client, 50, 632, 12],
        [$location, 50, 616, 11],
        ['Qty', 50, 580, 10],
        ['Description', 95, 580, 10],
        ['Unit Price', 380, 580, 10],
        ['Total', 475, 580, 10],
        ['1', 50, 555, 11],
        ['Monthly Services Charges for the Month of ' . $monthYear, 95, 555, 10],
        ['PKR ' . $amount, 370, 555, 10],
        ['PKR ' . $amount, 465, 555, 10],
        ['Total', 370, 515, 12],
        ['PKR ' . $amount, 465, 515, 12],
        ['Grand Total', 350, 493, 12],
        ['PKR ' . $amount, 465, 493, 12],
        ['Thank you for your business.', 50, 440, 11],
    ];
