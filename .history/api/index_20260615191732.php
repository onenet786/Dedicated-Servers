...
function invoice_pdf(array $server): string
{
    ...
    // Colors: dark blue header, light grey alternate rows, black text
    $pdf = begin_pdf();

    // === DARK HEADER BAR ===
    // Fill dark rectangle for company header
    rect_fill($pdf, 0, 735, 612, 107, '0.063 0.094 0.149'); // #111827
    // Company name - white
    text($pdf, 'Invoice', 306, 783, 20, '/F1', '1 1 1');
    text($pdf, 'OneNet Solutions', 50, 752, 14, '/F1', '1 1 1');
    text($pdf, 'NTN# 1443348-6', 50, 733, 9, '/F1', '0.75 0.75 0.75');
    text($pdf, 'Shop No.1, 83-D, The Mall, Lahore', 50, 717, 9, '/F1', '0.75 0.75 0.75');
    text($pdf, '+923214424625', 50, 703, 9, '/F1', '0.75 0.75 0.75');

    // === INVOICE INFO SECTION ===
    // Light grey separator line
    line($pdf, 50, 688, 562, 688, '0.8 0.8 0.8');
    text($pdf, 'Dated: ' . $renewalFormatted, 50, 668, 10, '/F1', '0 0 0');
    text($pdf, 'Invoice No.: ' . $invoiceNo, 310, 668, 10, '/F1', '0 0 0');

    // === BILL TO ===
    text($pdf, 'Bill To:', 50, 638, 10, '/F1', '0.4 0.4 0.4');
    text($pdf, $client, 50, 620, 12, '/F1B', '0 0 0');
    text($pdf, $name, 50, 604, 10, '/F1', '0 0 0');
    if ($location !== '') {
        text($pdf, $location, 50, 588, 10, '/F1', '0 0 0');
    }
    ...
    $tableRight = 572;
    $col1 = 60;   // Qty
    $col2 = 110;  // Description
    $col3 = 380;  // Unit Price
    $col4 = 475;  // Total
    $rowH = 20;

    // Dark header row
    rect_fill($pdf, $tableLeft, $tableTop - $rowH, $tableRight - $tableLeft, $rowH, '0.063 0.094 0.149'); // #111827
    text($pdf, 'Qty', $col1, $tableTop - 6, 9, '/F1B', '1 1 1');
    text($pdf, 'Description', $col2, $tableTop - 6, 9, '/F1B', '1 1 1');
    text($pdf, 'Unit Price', $col3, $tableTop - 6, 9, '/F1B', '1 1 1');
    text($pdf, 'Total', $col4, $tableTop - 6, 9, '/F1B', '1 1 1');

    // === TABLE ROW ===
    $rowY = $tableTop - $rowH - 2;
    // Alternating light grey row background
    rect_fill($pdf, $tableLeft, $rowY - 18, $tableRight - $tableLeft, 18, '0.965 0.965 0.965');
    // Row text
    text($pdf, '1', $col1, $rowY - 4, 10, '/F1', '0 0 0');
    text($pdf, 'Monthly Services Charges for', $col2, $rowY - 4, 9, '/F1', '0 0 0');
    text($pdf, 'the Month of ' . $monthYear, $col2, $rowY - 16, 9, '/F1', '0 0 0');
    text($pdf, 'PKR ' . $amount, $col3, $rowY - 4, 10, '/F1', '0 0 0');
    text($pdf, 'PKR ' . $amount, $col4, $rowY - 4, 10, '/F1', '0 0 0');

    // Table grid lines
    // Horizontal lines
    line($pdf, $tableLeft, $tableTop, $tableRight, $tableTop, '0.8 0.8 0.8');
    line($pdf, $tableLeft, $tableTop - $rowH, $tableRight, $tableTop - $rowH, '0.8 0.8 0.8');
    line($pdf, $tableLeft, $rowY - 18, $tableRight, $rowY - 18, '0.8 0.8 0.8');
    line($pdf, $tableLeft, $rowY - 18 - 22, $tableRight, $rowY - 18 - 22, '0.8 0.8 0.8');

    // Vertical lines
    $vlineY2 = $rowY - 18 - 22;
    $vlineY1 = $tableTop;
    line($pdf, $tableLeft, $vlineY1, $tableLeft, $vlineY2, '0.8 0.8 0.8');
    line($pdf, 85, $vlineY1, 85, $vlineY2, '0.8 0.8 0.8');
    line($pdf, $col3 - 20, $vlineY1, $col3 - 20, $vlineY2, '0.8 0.8 0.8');
    line($pdf, $col4 - 15, $vlineY1, $col4 - 15, $vlineY2, '0.8 0.8 0.8');
    line($pdf, $tableRight, $vlineY1, $tableRight, $vlineY2, '0.8 0.8 0.8');

    // === TOTALS ===
    $totalY = $vlineY2 - 10;
    rect_fill($pdf, $tableLeft, $totalY - 18, $tableRight - $tableLeft, 18, '0.965 0.965 0.965');
    text($pdf, 'Total', 360, $totalY - 4, 11, '/F1B', '0 0 0');
    text($pdf, 'PKR ' . $amount, $col4, $totalY - 4, 11, '/F1B', '0 0 0');
    line($pdf, $tableLeft, $totalY - 18, $tableRight, $totalY - 18, '0.8 0.8 0.8');

    $grandY = $totalY - 22;
    rect_fill($pdf, $tableLeft, $grandY - 18, $tableRight - $tableLeft, 18, '0.063 0.094 0.149'); // dark
    text($pdf, 'Grand Total', 340, $grandY - 4, 11, '/F1B', '1 1 1');
    text($pdf, 'PKR ' . $amount, $col4, $grandY - 4, 11, '/F1B', '1 1 1');
    line($pdf, $tableLeft, $grandY - 18, $tableRight, $grandY - 18, '0.8 0.8 0.8');

    // === FOOTER ===
    text($pdf, 'Thank you for your business.', 50, $grandY - 50, 10, '/F1', '0.4 0.4 0.4');
